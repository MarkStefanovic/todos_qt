...
def _config(
    *,
    path: pathlib.Path = fs.config_path(),
) -> dict[str, typing.Any]:
...
def add_holidays(
    *,
    config_path: pathlib.Path = fs.config_path(),
) -> bool:
...
def db_url(
    *,
    secrets_path: pathlib.Path = fs.secrets_path(),
    assets_folder: pathlib.Path = fs.assets_folder(),
) -> str:
